...
public class CartServiceImpl implements CartService {
    ...
    public CartResponseDto addItemToCart(AddToCartRequestDto requestDto) {
        ...
        Producto producto = productoRepository.findById(requestDto.idProducto())
                .orElseThrow(() -> new ResourceNotFoundException("Producto no encontrado con ID: " + requestDto.idProducto()));
        ...
        if (carrito.getSede() == null) {
            ...
            throw new IllegalArgumentException("No puedes añadir productos de diferentes sedes. Tu carrito actual está vinculado a la sede: " + carrito.getSede().getNombreSede());
        }

        StockSede stock = stockSedeRepository.findById(new StockSedeId(requestDto.idProducto(), requestDto.idSede()))
                .orElseThrow(() -> new ResourceNotFoundException("No hay stock registrado para este producto en la sede seleccionada."));
        ...
        if (stock.getCantidad() < cantidadTotalEnCarrito) {
            throw new IllegalArgumentException("Stock insuficiente. Stock disponible: " + stock.getCantidad() + ", intentas añadir: " + cantidadTotalEnCarrito);
        }

        if (itemExistenteOpt.isPresent()) {
            itemExistenteOpt.get().setCantidad(cantidadTotalEnCarrito);
            log.info("Cantidad actualizada para Producto (ID: {}) en Carrito (ID: {})", requestDto.idProducto(), carrito.getId());
        } else {
    ...
    private Usuario getAuthenticatedUser() {
        ...
        return usuarioRepository.findByEmail(userEmail)
                .orElseThrow(() -> new UsernameNotFoundException("Usuario no encontrado en el contexto de seguridad: " + userEmail));
    }
